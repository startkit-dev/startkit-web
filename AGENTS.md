...
This is a TanStack Start application with React, using pnpm as the package
...
- Vite Plus (vite 8, oxfmt, oxlint, vitest)
...
```bash
pnpm run check           # Run all checks (format, lint, typecheck, tests)
...
pnpm run db:migrate      # Apply database migrations (local)
pnpm run db:migrate:prod # Apply database migrations (production)
...
```bash
pnpm run setup           # Initial project setup (install, env, migrate)
pnpm run outdated        # Check for package updates interactively
pnpm run typegen         # Generate Cloudflare Worker types
```
